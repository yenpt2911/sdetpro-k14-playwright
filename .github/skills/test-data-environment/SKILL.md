...
Never inline `Date.now()`/`Math.random()` directly inside a test's `test()` title string — that breaks retries (see `flaky-test-triage` skill's "dynamic title" pitfall). Instead:
...
- **Dynamic value in test title**: causes `Test not found in the worker process` on retry — generate uniqueness in test-data, not in the title string (cross-reference `flaky-test-triage`).
